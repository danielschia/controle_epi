...
```bash
python manage.py createsuperuser --username=controle_epi --email=controle_epi@example.local
```
...
for username, email in [('gerente1','gerente1@example.local'), ('gerente2','gerente2@example.local')]:
...
for email in ['gerente1@example.local','gerente2@example.local']:
...
- `DJANGO_SUPERUSER_EMAIL` — email do superuser (padrão: `controle_epi@example.local`).
...
- Criar superuser: `python manage.py createsuperuser --username=controle_epi --email=controle_epi@example.local`
...
- Por padrão ele cria `gerente1` e `gerente2` com emails `gerente1@example.local` e `gerente2@example.local`.
